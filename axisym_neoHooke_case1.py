...
mesh = Mesh()
XDMFFile("meshes/data/mesh1.xdmf").read(mesh)
...
class FiberDomain(SubDomain):
    def inside(self, x, on_boundary):
        return between(x[0], (0.0, Rf))

class MatrixDomain(SubDomain):
    def inside(self, x, on_boundary):
        return between(x[0], (Rf, R))
axisbndry = Axis()
rightbndry = Right()
bottombndry = Bottom()
topbndry = Top()
...
resultsdir = f'results_case1_kapparatio_{float(kapparatio)}_n_{float(n)}_muf_{float(muf)}'
